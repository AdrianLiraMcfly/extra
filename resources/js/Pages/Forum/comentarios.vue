<template>
    <AuthenticatedLayout>
        <div v-if="$page.props.flash.message && showAlert"
            class="relative p-6 mb-4 text-green-700 bg-green-100 border-0 rounded-lg shadow-lg dark:bg-green-200 dark:text-green-800"
            role="alert">
                        ...
                        <div class="card-body">
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-for="comentario in comentarios" :key="comentario.id">
                                        ...
                                        <td>
                                            <button-modal v-if="$page.props.auth.user.id == comentario.user.id"
                                                id_Modal="modalEditarComentario" class="btn btn-warning" boton="Editar"
                                                @click="editComentario(comentario.id, comentario.comentario)"></button-modal>
                                            <button-modal v-if="$page.props.auth.user.id == comentario.user.id"
                                                id_Modal="modalEliminarComentario" class="btn btn-danger"
                                                boton="Eliminar"
                                                @click="obtenerComentario(comentario.id)"></button-modal>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <div class="mb-3">
                                <input type="text" class="form-control" id="comentario" v-model="form.comentario"
                                    ...
                                    style="border: 2px solid blue; background-color: #e6f7ff;">
                                <button class="btn btn-primary" @click="crearComentario">Crear</button>
                                <p v-if="errors.comentario" class="text-red-500 text-xs italic">{{ errors.comentario }}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <Modal id_modal="modalEditarComentario" titulo="Editar Comentario">
            <form @submit.prevent="editarComentario">
                <div class="mb-3">
                    ...
                    <p v-if="errors.comentario" class="text-red-500 text-xs italic">{{ errors.comentario }}</p>
                </div>
        ...
        <Modal id_modal="modalEliminarComentario" titulo="Eliminar Comentario">
            ...
            <button type="submit" class="btn btn-danger" @click="eliminarComentario"
